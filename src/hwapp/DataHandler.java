...
public class DataHandler {
     ...
     * @param host HOST-IP oder gueltiger Verbindungsname (eg. localhost) zum
     * MySQL-Server
     * @param port Kommunikationsport fuer Verbindung zum MySQL-Server (eg.
     * 3306)
     * @param databaseName Name der vorhandenen Datenbank fuer Verwaltung der
     * hwapp-Daten
     ...
     * @param userPassword Password fuer Login in MySQL Datenbank fuer den
     * Benutzer
     * @param userDarfMMC Darf der Benutzer die ManagementKonsole starten? 0 =
     * nein, 1 = ja
    ...
    /**
     * @return Es werden alle Datensaetze der Tabelle users in einem ResultSet
     * zuruck gegeben.
     */
    ...
    /**
     * @return Es werden alle Datensaetze der Tabelle users in einem StringArray
     * zuruck gegeben.
     */
    ...
    /**
     * @param userLogin Login-Name des Benutzers (eg. Email-Adresse)
     * @return gibt true zurueck, wenn mindestens ein Benutzer mit dem
     * Login-Namen existiert
     */
    ...
    /**
     * @return Es werden alle Datensaetze der Tabelle tickets in einem ResultSet
     * zuruck gegeben.
     */
    ...
    //Start ma 14.06.2018
    //blob nachprüfen 
    public ResultSet getAllAnswersForThisTicket(int ticketID) {

        ResultSet rs = null;
        this.logger.addToLogFile(this.getClass().getSimpleName(), "Lese alle Datensaetze der Tabelle tickets ...", LogLevel.LoggingLevel.DEBUG);
        this.mySQL.connect();
        if (this.mySQL.isConnected()) {
            rs = this.mySQL.doQuery("SELECT * FROM ticket WHERE ticketID = " + ticketID);
            this.mySQL.disconnect();
        } else {
            this.logger.addToLogFile(this.getClass().getSimpleName(), "Lesen aller Datensaetze tickets nicht moeglich ... Konnektion zur Datenbank nicht gelungen.", LogLevel.LoggingLevel.ERROR);
        }
    ...
    public boolean getMediaForThisTicket(int ticketID) {

        //Übergabe prüfen Blob / Boolean = wie behandelt das ein Handy 
        boolean feedback = false;
        //Pruefungen fehlen noch
        this.mySQL.connect();
        this.mySQL.doUpdate("SELECT * FROM media WHERE ticketID = " + ticketID);
        this.mySQL.disconnect();
        feedback = true;
        return feedback;
    }
    ...
    //blob nachprüfen 
    public boolean setMedia(Blob mediadaten) {

        boolean feedback = false;
        //Pruefungen fehlen noch
        this.mySQL.connect();
        this.mySQL.doUpdate("INSERT INTO foto (fotoid,fotoImage) VALUES (NULL, '" + mediadaten + "'");
        this.mySQL.disconnect();
        feedback = true;
        return feedback;
    ...
    public boolean deleteMedia(int mediaID) {
        ...
        //Pruefungen fehlen noch
        this.mySQL.connect();
        this.mySQL.doUpdate("delete FROM tickets WHERE mediaID = '" + mediaID + "'");
        this.mySQL.disconnect();
        feedback = true;
        return feedback;
    ...
    public boolean setNewKategorie(String kategoriedaten) {

        boolean feedback = false;
        //Pruefungen fehlen noch
        this.mySQL.connect();
        this.mySQL.doUpdate("INSERT INTO kategorie (kategorieID,kategorieName) VALUES (NULL, '" + kategoriedaten + "'");
        this.mySQL.disconnect();
        feedback = true;
        return feedback;
    ...
    public ResultSet getAllKategorie() {
        ResultSet rs = null;
        this.logger.addToLogFile(this.getClass().getSimpleName(), "Lese alle Datensaetze der Tabelle kategorie ...", LogLevel.LoggingLevel.DEBUG);
        this.mySQL.connect();
        if (this.mySQL.isConnected()) {
            rs = this.mySQL.doQuery("SELECT * FROM kategorie");
            this.mySQL.disconnect();
        } else {
            this.logger.addToLogFile(this.getClass().getSimpleName(), "Lesen aller Datensaetze kategorie nicht moeglich ... Konnektion zur Datenbank nicht gelungen.", LogLevel.LoggingLevel.ERROR);
        }
        return rs;
    }

    public ResultSet getKategorie(int kategorieID) {

        ResultSet rs = null;
        this.logger.addToLogFile(this.getClass().getSimpleName(), "Lese alle Datensaetze der Tabelle kategorie ...", LogLevel.LoggingLevel.DEBUG);
        this.mySQL.connect();
        if (this.mySQL.isConnected()) {
            rs = this.mySQL.doQuery("SELECT * FROM kategorie WHERE kategorieID = " + kategorieID);
            this.mySQL.disconnect();
        } else {
            this.logger.addToLogFile(this.getClass().getSimpleName(), "Lesen aller Datensaetze kategorie nicht moeglich ... Konnektion zur Datenbank nicht gelungen.", LogLevel.LoggingLevel.ERROR);
        }
    ...
    public boolean deleteKategorie(int kategorieID) {

        boolean feedback = false;
        //Pruefungen fehlen noch
        this.mySQL.connect();
        this.mySQL.doUpdate("delete FROM kategorie WHERE kategorieID = " + kategorieID);
        this.mySQL.disconnect();
        feedback = true;
        return feedback;
    ...
    public boolean setNewSubKategorie(Blob subKategorieDaten) {

        boolean feedback = false;
        //Pruefungen fehlen noch
        this.mySQL.connect();
        this.mySQL.doUpdate("INSERT INTO subkategorie (subkategorieID,subkategorieName) VALUES (NULL, '" + subKategorieDaten + "'");
        this.mySQL.disconnect();
        feedback = true;
        return feedback;
    ...
    public ResultSet getSubKategorieForThisKategorie(int kategorie) {
        ResultSet rs = null;
        this.logger.addToLogFile(this.getClass().getSimpleName(), "Lese alle Datensaetze der Tabelle kategorie ...", LogLevel.LoggingLevel.DEBUG);
        this.mySQL.connect();
        if (this.mySQL.isConnected()) {
            rs = this.mySQL.doQuery("SELECT * FROM subkategorie WHERE kategorieID = " + kategorie);
            this.mySQL.disconnect();
        } else {
            this.logger.addToLogFile(this.getClass().getSimpleName(), "Lesen aller Datensaetze kategorie nicht moeglich ... Konnektion zur Datenbank nicht gelungen.", LogLevel.LoggingLevel.ERROR);
        }
